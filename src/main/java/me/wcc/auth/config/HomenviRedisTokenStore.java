...
public class HomenviRedisTokenStore implements TokenStore {
    ...
    @Autowired
    public HomenviRedisTokenStore(RedisConnectionFactory connectionFactory) {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        OAuth2AccessToken accessToken = findTokenByKey(serializedKey);
        if (accessToken != null) {
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
        ...
        return findTokensByKey(approvalKey);
    }

    @Override
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        byte[] key = serializeKey(CLIENT_ID_TO_ACCESS + clientId);
        return findTokensByKey(key);
    }

    private Collection<OAuth2AccessToken> findTokensByKey(byte[] key) {
        List<byte[]> byteList;
        RedisConnection conn = getConnection();
        try {
    ...
    private OAuth2AccessToken findTokenByKey(byte[] key) {
        byte[] bytes;
        RedisConnection conn = getConnection();
        try {
            bytes = conn.get(key);
